...
public class ShortestRoutesII {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for(int i=0;i<m;i++){
            ...
            long wt=in.nextLong();
            distance[src][dest]=Math.min(distance[src][dest],wt);
            distance[dest][src]=Math.min(distance[dest][src],wt);
        }
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                for(int k=0;k<n;k++){
        ...
        for(int i=0;i<q;i++){
            int src=in.nextInt()-1;
            int dest=in.nextInt()-1;
            if(distance[src][dest] == 1000000000000l){
