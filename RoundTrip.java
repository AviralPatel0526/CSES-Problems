...
public class RoundTrip {
    ...
    static boolean dfs(int node, int parent, int vis[], ArrayList<ArrayList<Integer>> adj, ArrayList<Integer> list) {
        vis[node] = 1;
        list.add(node);
        
        for (int adjacentNode : adj.get(node)) {
            if (vis[adjacentNode] == 0) {
                if (dfs(adjacentNode, node, vis, adj, list)) return true;
            } 
            else if (adjacentNode != parent) { // Cycle detected
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            if (vis[i] == 0) { // Check each component
                list.clear();
                if (dfs(i, -1, vis, adj, list)) {
                    foundCycle = true;
                    break;
                }
            }
