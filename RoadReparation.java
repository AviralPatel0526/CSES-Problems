...
public class RoadReparation {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for(int i=0;i<m;i++){
            ...
            int d=in.nextInt()-1;
            long wt=in.nextInt();
            list.add(new Triplet(s, d, wt));
        }
        Collections.sort(list,(t1,t2)->Long.compare(t1.wt, t2.wt));
        dsu obj=new dsu(n);
        long ans=0;
        for(int i=0;i<m;i++){
            int s=list.get(i).s;
            int d=list.get(i).d;
            long wt=list.get(i).wt;
            int ulps=obj.findParent(s);
            int ulpd=obj.findParent(d);
            if(ulps != ulpd){
                obj.unionBySize(s, d);
                ans+=wt;
            }
